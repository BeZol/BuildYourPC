...
class BuildTableViewCell: InfoTableViewCell, UIScrollViewDelegate {
    
    override class var cellID: String {
        
        get{
            return "kBuildInfoTableViewCellID"
        }
    }
    ...
    override func willDisplay(object: Any?) {
        ...
        guard let build = object as? Build else{
            return
        }
        
        self.lblTitle.text = build.name
        ...
        if let formattedPrice = formatter.string(from: build.price as NSNumber) {
        ...
        self.lblDescription.text = build.buildDescription
        
        self.avatarImageView.sd_setImage(with: URL(string: build.imageURL ?? ""))
                
        // Find containerView height constrait
        // Set 0 if there isn't any images
        if let heightConstraint = (containerView.constraints.filter{$0.firstAttribute == .height}.first) {
            if build.productImages.count == 0 {
        ...
        scrollView.contentSize = CGSize(width: scrollView.frame.size.width * CGFloat(build.productImages.count), height: scrollView.frame.size.height)
        ...
        for index in 0..<build.productImages.count {
            ...
            subView.tag = 333
            subView.sd_setImage(with: URL(string: build.productImages[index]))
            subView.contentMode = .scaleAspectFit
            scrollView.addSubview(subView)
        }

        self.pageControl.numberOfPages = build.productImages.count
    }
